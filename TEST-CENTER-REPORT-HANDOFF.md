# Test Center Report — Prompt Handoff (v4.2)
...
```ts
{
  ...
  lessons: Array<{
    ...
    score: number;
    total: number;
    skills?: Array<{ skill: string; domain: string; correct: number; total: number }>;  // v4 신규
  }>;
  skills?: Array<{ skill: string; domain: string; correct: number; total: number }>;   // 세션 전체 합산
}
```
...
| `lessonBlocks` | 모듈별 `제목: score/total (pct%)` + 스킬 상세 (정답률 낮은 순 상위 4개) |
...
- **SuperfastSAT 코칭 철학** 블록 포함: 학습 사이클(레슨→SH→TC), 모듈2 적응형 구조 설명
...
**캐시 키 구성 요소**: `{ curriculumTitle, curriculumDomain, totalScore, totalProblems, lessons(title/score/total), skills(sorted), shCrossRef, coachFeedback, vocabContext }`
...
test_center_lesson_attempts (per lesson)
  └─ test_center_unit_attempts (per unit → skill 집계)   ← v4 신규
  └─ test_center_session (started_at → KST date)
  └─ lessons (lesson_id → title)
  └─ curricula (curriculum_id → title, domain)
  └─ units (unit_id → skill, domain)

→ tcSkillsByLessonAttempt: 레슨별 스킬 집계
→ tcBySession: { lessons[with skills], curriculumTitle, curriculumDomain, skills }
→ 레슨 정렬(RW→Math, 모듈 번호순)
→ shCrossRef 계산 (도메인별 SH↔TC 격차)
→ weakModuleLines 사전 계산
→ generateTestCenterNarrative() 호출
