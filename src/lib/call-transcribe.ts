...
const LABEL_PROMPT = `다음은 SAT 학원 세일즈 담당자와 고객(학생/학부모)의 전화 상담 전사다.
(스피커폰을 마이크로 녹음·전사한 것이라 오탈자·잡음 섞임이 있을 수 있다.)
화자를 구분해 각 발화 앞에 "세일즈 담당자:" 또는 "고객:" 라벨을 붙여 대화 형식으로 다시 정리해라.
규칙:
- 내용·표현은 최대한 그대로 두고 화자만 구분한다(요약·생략 금지).
- 누가 말했는지는 문맥으로 추정한다(세일즈 담당자는 학원 측, 고객은 학생/학부모).
...
- 전사에 없는 내용을 추측하지 말 것.
- "고객" 발화에서 니즈·우려를, "세일즈 담당자" 발화에서 합의·다음 액션을 우선 파악한다.
...
/** Gemini로 화자(세일즈 담당자/고객)를 구분한 전사로 재구성. */
