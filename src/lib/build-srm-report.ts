    ...
    messages: [
      {
        ...
        content: [
          ...
          '- "~점을/것을 강조합니다" → "~점을/것을 보여줍니다"',
          '- "~점을/것을 나타냅니다" → "~점을/것을 보여줍니다"',
          '- "~점을/것을 의미합니다" → "~점을/것을 보여줍니다"',
          '- 영어 구절이 따옴표 안에 있으면 그 내용을 한국어로 풀어 씁니다. 예: "\'high cost\'" → "비용 문제"',
          '',
    ...
    messages: [
      {
        ...
        content: isCompact ? [
          ...
          '- 오류 유형 해석 우선순위:',
          '  1순위 — [Eden 대화 인사이트]가 있으면: 인사이트에서 파악한 약점을 학부모가 이해할 수 있는 한국어로 풀어 씁니다.',
          '     영어 구절을 따옴표로 그대로 인용하지 않습니다. 예: "\'high cost and resource scarcity\'와 같은 접근" (X)',
          '     대신 학생이 어떤 내용을 잘못 이해했는지 한국어로 설명합니다. 예: "에너지 비용 문제로 잘못 파악했습니다" (O)',
          '     "[Eden~]에서는" 직접 언급 금지. 학부모가 아이가 공부하는 장면을 그릴 수 있어야 합니다.',
          '     "~와 같은 접근이 나타났습니다" 같은 추상적 표현 금지. 학생이 구체적으로 무엇을 잘못 이해했는지 서술합니다.',
          ...
          '  · 빠른 오답(충동적) 지배 → "빠르게 답하지만 틀리는 경우가 많습니다. 패턴을 성급하게 적용하고 있습니다"',
          ...
          '- [행동 패턴 분석] 내 "확신 오류"가 있으면: "자신 있게 선택했지만 N건을 틀렸습니다. 방향 자체가 어긋나 있어 다음 수업에서 어디서 판단이 틀리는지를 짚어야 합니다"처럼 씁니다. "확신 오류"라는 용어를 출력에 그대로 쓰지 않습니다.',
          '- "불확실 정답"이 많으면: "맞추긴 했지만 아직 확신 없이 고르는 단계입니다"처럼 씁니다. 이 스킬을 "완전히 체화됐습니다"라고 쓰지 않습니다. "불확실 정답"이라는 용어를 출력에 그대로 쓰지 않습니다.',
          '- [테스트센터 교차]가 있으면 연습-검증 격차를 해석합니다. (격차 >10%p → 압박 하 적용 훈련 필요)',
          '- [최근 단어 학습]이 있고 Words in Context 스킬이 있으면: 단어 이름 나열 대신 패턴으로 씁니다.',
          '',
          '셋째 단락 — 다음 수업 방향 (1문장):',
          '- 이 문장의 핵심 구조: "이번 세션에서 확인된 [구체적 약점]을 코치에게 전달해 다음 수업에서 [어떻게] 다루겠습니다."',
          '- Eden(나)이 이번 스터디홀에서 파악한 약점을 코치에게 알려서 다음 수업에 반영하겠다는 내용을 담습니다.',
          '- [코치 피드백]이 있으면 이번 세션 데이터와 교차되는 약점을 골라 "이 부분을 코치에게 전달해 다음 수업에서 집중하겠습니다"처럼 씁니다.',
...
function inferDomainFromLessons(lessons: { title?: string; total: number }[]): string | undefined {
  ...
  // Fallback: RW 모듈은 27문항, Math 모듈은 22문항
  const hasRW = lessons.some(l => l.total === 27);
  const hasMath = lessons.some(l => l.total === 22);
  if (hasRW && !hasMath) return 'RW';
  if (hasMath && !hasRW) return 'Math';
  return undefined;
}
    ...
    messages: [
      {
        ...
        content: [
          ...
          '- 코치가 우려했던 부분이 이번 결과에서 확인됐는지, 아니면 개선됐는지를 학부모에게 전달합니다.',
          '',
          'SAT 시험 구조:',
          ...
          '- 첫 문장은 "[테스트/커리큘럼명]에서" 또는 "[RW/Math] 결과를 보면" 처럼 결과를 직접 서술하며 시작합니다. "오늘"로 시작하지 않습니다.',
          ...
          '- [코치 피드백] 항목이 있으면 마지막 문장은 두 방향 중 이번 결과와 교차하는 것을 선택합니다:',
          '  방향 A (수업→테스트센터): 수업에서 다룬 내용이 이번 테스트 결과에서 확인됐다면 그 연결을 전달합니다.',
          '  방향 B (테스트센터→수업): 이번 테스트에서 드러난 약점이 다음 수업에서 보완될 것이라는 방향으로 씁니다.',
          '- [코치 피드백]이 없으면 마지막 문장은 이번 결과에서 도출한 방향입니다.',
          ...
          '- "오늘"이란 단어를 출력에 쓰지 않습니다. 학부모가 리포트를 보는 시점은 당일이 아닐 수 있으므로, "이번 세션에서", "이번 테스트에서" 등으로 바꿉니다.',
          '- "오류가 납니다", "오류가 발생했습니다", "오류를 보였습니다" 같은 추상적 표현은 쓰지 않습니다. 대신 "틀렸습니다", "실수가 있었습니다", 또는 구체적인 상황으로 씁니다.',
          '- "확신 오류", "막힌 패턴", "성급한 오답" 같은 내부 용어를 출력에 그대로 쓰지 않습니다. 반드시 위에 안내된 방식으로 풀어서 씁니다.',
  ...
  await Promise.all(Array.from(shByDate.entries()).map(async ([date, stats]) => {
    ...
    if (!narrative) {
      if (stats.totalProblems === 0) {
        ...
        narrative = await generateStudyHallNarrative({ durationMinutes: stats.totalMinutes, totalProblems: stats.totalProblems, correctCount: stats.correctCount, accuracy, skills }, tcCrossRef.length ? tcCrossRef : undefined, coachFeedback, edenInsight, vocabContext, skillKnowledgeMap);
