...
/** 저장된 최신 refresh_token을 반환한다. 없거나 오류면 null. */
...
/** 회전된 새 refresh_token을 저장한다. 실패는 치명적이지 않다(다음 갱신에서 재시도). */
export async function writeStoredRefreshToken(refreshToken: string): Promise<void> {
  try {
    ...
    await supabaseAdmin
      .from('integration_tokens')
      .upsert(
        { provider: PROVIDER, refresh_token: refreshToken, updated_at: new Date().toISOString() },
        { onConflict: 'provider' }
      );
  } catch {
    /* noop */
