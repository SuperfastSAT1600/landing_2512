  ...
  if (variant === 'card') {
    const cardInner = (
      <>
        ...
        <span className="text-xs sm:text-sm font-medium text-accent-glow">{name}</span>
        {href && <ExternalLink className="w-3.5 h-3.5 text-accent-glow/70 flex-shrink-0 ml-auto" />}
      </>
    );
    const cardClass =
      'flex items-center gap-2.5 bg-accent/10 border border-accent/20 rounded-lg px-3.5 py-3';
    if (href) {
      return (
        <a
          href={href}
          ...
          className={`${cardClass} hover:bg-accent/20 hover:border-accent/40 transition-colors`}
        >
          {cardInner}
        </a>
      );
