  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div
        ...
        style={{
          border: `1.5px dashed ${isDragging || isHovered ? '#6085FF' : '#27272a'}`,
          borderRadius: 8,
          padding: preview ? 0 : '20px 12px',
          background: isDragging ? 'rgba(96,133,255,0.06)' : '#000',
          cursor: 'pointer',
      ...
      >
        {preview ? (
          ...
          <>
            <span style={{ fontSize: 12, color: '#52525b' }}>클릭·드래그·Ctrl+V</span>
            <span style={{ fontSize: 11, color: '#3f3f46' }}>PNG, JPG, WEBP · 최대 5MB</span>
          </>
        )}
      </div>
      {value && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 11, color: '#52525b' }}>{value.name || '붙여넣기 이미지'}</span>
          <button
            type="button"
