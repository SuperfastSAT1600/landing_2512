...
import {
    Image as ImageIcon, Link2, Smile,
    ...
    return (
        ...
        <div className={`fixed top-16 w-full z-[90] bg-[#151719]/95 backdrop-blur-sm border-b border-white/5 flex items-center gap-0.5 px-4 h-11 transition-all duration-300 ${showSettings ? 'pr-[332px]' : ''}`}>
            ...
            <button
                ...
                title="이모지 삽입"
                className={`w-8 h-8 flex items-center justify-center rounded hover:bg-white/10 transition-colors ${showEmoji ? 'bg-white/15 text-white' : 'text-gray-400 hover:text-white'}`}
            >
                <Smile size={14} />
            </button>
        </div>
