...
/**
 * 이탈 리드풀 상단의 플레이 컨텍스트 바 — (구 AI 검색 자리)
 * 플레이를 하나 고르면 리드 목록에 타겟 배지가 붙고, 선택한 리드를 그 플레이에 담을 수 있다.
 */
    ...
    <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-3 space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {active.length > 0 ? (
          ...
          >
            <option value="">플레이 선택 안 함</option>
            {active.map((p) => (
          ...
          <span className="text-[11px] text-gray-500">진행 중인 플레이가 없습니다.</span>
        )}
      ...
      {selected && (
        <p className="text-[11px] text-gray-600">
          ...
          <span className="ml-2 text-gray-400">
            아래에서 리드를 선택해 이 플레이에 추가할 수 있습니다.
          </span>
        </p>
      )}
    </div>
