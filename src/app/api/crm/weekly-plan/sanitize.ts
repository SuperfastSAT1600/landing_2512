...
/** DB 행 → WeeklyPlan. 마이그레이션 112 이전에 만들어진 행도 기본값으로 채운다. */
export function normalizePlanRow(row: Record<string, unknown> | null | undefined): WeeklyPlan | null {
  ...
  return {
    ...
    actions: Array.isArray(row.actions) ? (row.actions as WeeklyPlanAction[]) : [],
    focus_strategies: Array.isArray(row.focus_strategies)
