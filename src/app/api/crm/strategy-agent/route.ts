...
const PROACTIVE_SEED = `방금 **이번 달 인입한 리드 코호트**의 퍼널 추이를 점검했다. 위 [KPI 건강 진단]을 1차 근거로, 이 리드들이 인입→컨택→상담→진단→결제로 가는 길에서 **어디서 가장 많이 막히고 빠지는지(드롭오프)**를 중심으로, 지금 가장 시급한 1~3개 영역을 골라 보고하라.
