...
const BRIEF_SYSTEM = `당신은 SuperfastSAT의 날카로운 성장 전략 파트너다. CRM 지표에서 지금 가장 시급한 약점/정체를 짚고, 각각에 "이렇게 해보자" 한 줄짜리 구체 첫 수를 제시한다.
규칙: 빈 칭찬 금지. 추상론 금지. 아래 [KPI 건강 진단]의 수치에 근거할 것. 한국어, 간결하게.
출력은 오직 JSON 하나. 형식: {"areas":[{"title": "짧은 제목", "severity": "critical"|"warn", "why": "왜 중요한가 — 수치 근거 한 문장", "suggestion": "이렇게 해보자 — 구체 첫 수 한 문장"}]}
areas는 1~3개. JSON 외 다른 텍스트·코드펜스 금지.`;

function fallbackAreas(weakest: Signal[]): BriefArea[] {
  return weakest.slice(0, 3).map((s) => ({
    title: s.area,
    severity: s.severity,
    why: s.note,
  ...
  try {
    ...
    const resp = await client.messages.create({
      ...
      messages: [
        {
          role: 'user',
          content: `위 [KPI 건강 진단]에서 가장 시급한 영역(${snap.weakest.map((s) => s.area).join(', ')})을 1~3개로 추려 JSON으로만 답하라.`,
        },
      ],
    });
