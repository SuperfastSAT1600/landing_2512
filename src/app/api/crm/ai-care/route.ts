...
const SYSTEM_PROMPT = `아래는 학원 상담 직원이 작성한 상담 메모입니다. 이 메모를 바탕으로, 학부모에게 보내는 상담 내용 요약 메시지를 작성해 주세요.
...
4. 주의사항:
   - 직원 내부 메모 특유의 표현(예: "B2B 파트너는 아님", "어머님이 정확한 점수는 모르심" 등)은 학부모 친화적 언어로 자연스럽게 변환
   - 민감한 정보(ADHD 등)는 사실 그대로 적되, 배려 있는 표현으로 순화
   - 내부 검토 중인 사항과 확정된 사항을 명확히 구분
   - 한국어로 작성
   - **bold** 등 마크다운 문법 사용 금지. 강조가 필요하면 이모지를 활용
