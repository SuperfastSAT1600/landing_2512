...
export async function POST(req: NextRequest) {
  ...
  let query = supabaseAdmin
    ...
    .select('id, is_active, scope, first_used_at, instagram_id')
    .eq('code', codeStr);
  ...
  if (data.scope !== 'mathweb' && data.scope !== 'both') {
    return NextResponse.json(
      { error: { code: 'SCOPE_MISMATCH', message: 'Math Web 접근 권한이 없는 코드예요.' } },
      { status: 401 },
    );
  }
  ...
  return NextResponse.json({ data: { instagram_id: identifier }, meta: { requestId } });
}
