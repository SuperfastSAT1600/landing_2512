...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('❌ 환경변수를 확인해 주세요. .env.local 파일에 NEXT_PUBLIC_SUPABASE_URL 및 SUPABASE_SERVICE_ROLE_KEY가 필요합니다.')
  process.exit(1)
}

const raw = fs.readFileSync(
  '/workspace/content/posts/2026-08-31-georgetown-superscore-sat-strategy-landing.md',
  'utf8'
)
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        if (res.statusCode === 200 || res.statusCode === 201) {
          ...
          console.log('URL: https://superfastsat.com/posts/' + post.id)
          resolve(post)
        } else {
