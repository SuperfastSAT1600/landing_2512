...
- 근거 층위: 직접 경험 → 실제 데이터 → 인과 설명 → 독자 행동
...
- 참고 기준: 뉴스·안내 600~1,000자 / 전략·개념 1,200~2,500자 / 데이터 분석 2,000~3,500자
...
## TDS 작성 원칙 적용 (Ghost 버전)

Ghost는 SEO 우선이지만, 문장 수준에서는 TDS 원칙을 동일하게 적용한다.

- **핵심 먼저**: H2 바로 아래 첫 문장이 해당 섹션의 결론이어야 한다.
- **문단 길이**: 모바일 기준 3~5줄. 7줄 초과 시 분리.
- **능동·단정형**: "~할 수 있습니다" → "~합니다". 수동태 최소화.
- **부사 삭제**: "정말", "굉장히", "매우"는 팩트로 대체한다.
- **이미지 캡션**: Ghost에서는 일반 이탤릭(*출처: ...*) 사용. `<figcaption>` 대신.
- **하이라이트**: `<mark>` 태그 사용 금지 (Ghost 전용 애니메이션 없음). 대신 **볼드**로 강조.
