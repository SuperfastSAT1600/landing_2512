...
See [rules/charts.md](rules/charts.md) for data viz / animated charts.
See [rules/typography.md](rules/typography.md) for kinetic typography patterns.
See [rules/spring-physics.md](rules/spring-physics.md) for spring-based motion.
See [rules/messaging.md](rules/messaging.md) for chat/SMS-style scene composition.
See [rules/social-media.md](rules/social-media.md) for portrait/short-form formats (TikTok, Reels, Shorts).

## Educational video (K-12, SAT, undergrad)

See [rules/educational-structure.md](rules/educational-structure.md) for pedagogical segment ordering, cognitive load rules, audience-specific defaults, and series consistency.
See [rules/katex-math.md](rules/katex-math.md) for rendering LaTeX math equations with KaTeX (required for SAT/STEM).
See [rules/code-highlighting.md](rules/code-highlighting.md) for syntax-highlighted code in CS/programming tutorials using Shiki.
See [rules/quiz-interactions.md](rules/quiz-interactions.md) for SAT/AP multiple-choice reveal patterns (question → think pause → correct highlight → distractor X-out).
See [rules/whiteboard-handwriting.md](rules/whiteboard-handwriting.md) for stroke-by-stroke "teacher writing on a board" SVG animations.

## Free asset sources

See [rules/assets-library.md](rules/assets-library.md) for curated free, legally-usable fonts, icons, illustrations, Lottie, stock video, music/SFX, datasets, and math/science tools — with license notes for monetized vs non-commercial use.

## Scaling production

See [rules/lambda-rendering.md](rules/lambda-rendering.md) for batch-rendering a course on AWS Lambda — required when producing dozens+ videos or per-student personalized content.
