...
[섹션별 점검]: 각 H2 섹션에서 세 전제 조건(발견·단일주장·독자위치)이 구현됐는지 선언
  섹션 1 "[섹션명]": 독자 위치(독자가 갖는 오해/혼란) ___ / 이 섹션의 주장 ___ / 근거 ___
  섹션 2 "[섹션명]": 독자 위치 ___ / 주장 ___ / 근거 ___
  섹션 N: ...
...
| **② 섹션별 점검** | 골격의 [섹션별 점검]에서 각 H2 섹션이 독자 위치(오해/혼란)·단일 주장·근거를 갖추는가? | 해당 섹션에 데이터/수치 또는 인과 설명 추가 |
...
### 구조 점검 도구 (세 조건이 글에 구현됐는지 확인)

C→R→E→I는 설득의 공식이 아니라, 세 전제 조건이 각 섹션에 실현됐는지 점검하는 도구다.

```
[C — 혼란 장면] 독자가 지금 어디 있는지 — 독자가 실제로 겪는 구체적 혼란을 꺼낸다.
[R — 패턴 인식] 글쓴이의 발견 — 데이터로 혼란의 실체를 보여준다.
[E — 메커니즘] 하나의 주장을 뒷받침 — "왜 그런지" 인과를 설명한다.
...
### 섹션별 점검 (STEP 0.5 [섹션별 미니 아크] 실행)

각 H2 섹션은 포스팅 전체의 축소판이다. 세 전제 조건이 섹션 단위에서도 구현되어야 한다.
...
② 섹션 점검    (3점)
   3 = 전체 H2 섹션의 80%+ 가 독자 위치·단일 주장·근거를 갖춤
   2 = 50~79% 갖춤
   1 = 50% 미만 갖춤
   0 = 섹션별 점검 없음 (전체가 설명 나열)
